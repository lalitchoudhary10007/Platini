    ...
    return Observable.create(observer => {
      ...
      data.forEach(item => {
        insertRows.push([
          ...
          [item.clothesId, item.styleNumber, item.price, item.clothesDescription, item.tags, item.categoryId,
      ...
      this.db.executeSql('SELECT * FROM ProductCart WHERE clothesId=?', [clothID])
        .then(res => {
          ...
          if (res.rows.length == 0) {
            ...
            this.db.executeSql('INSERT INTO ProductCart VALUES(?,?)', [clothID, cartData])
              .then(res1 => {
            ...
            this.db.executeSql('UPDATE ProductCart SET cartData=? WHERE clothesId=?', [cartData, clothID])
              .then(updateres => {
