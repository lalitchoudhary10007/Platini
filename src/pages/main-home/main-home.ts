...
import { SessionHelperProvider, AppUtilsProvider, ProductsProvider, ConfigTablesProvider , ApiHelperProvider} from '../../providers/providers';
...
export class MainHomePage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, platform: Platform,public events: Events,private transfer: FileTransfer,public configProvider: ConfigTablesProvider,
               ...
               public shared: SharedhelperProvider,public appCtrl: App) {
  ...
  ionViewDidLoad() {
   ...
   SyncData() {
    ...
    let data = { UserId: 0, Mode: 0, SubMode: 7, DeviceInfo: 'Test Device', SyncDate: '2010-04-16 14:00:00.000' };
      data.UserId = 1;
      data.Mode = 0;
      data.SubMode = 7;
      data.DeviceInfo = 'Test Device';
      data.SyncDate = '2010-04-16 14:00:00.000';
      
      this.ApiHelper.RequestPostHttp(data, 'FullSync', true).then(result => {
        //here you have the analog success function from an ajax call 
        //use the received data
       
    
       console.log("**Terms" , result);
        this.configProvider.SaveBatchesInSQL(result.Terms).subscribe(res =>{
